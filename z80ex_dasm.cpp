	...
	if(dasm!=NULL)
	{
		...
		for(mpos=(dasm->mnemonic); *mpos && out_len < output_size; mpos++)
		{
			*stmp='\0';
			...
			{
				case '@':
					lo=readbyte_cb(addr++,bank);
					hi=readbyte_cb(addr++,bank);
					bytes+=2;
				...
				case '$':
				case '%':
					if(!have_disp) disp_u = readbyte_cb(addr++,bank);
					bytes++;
